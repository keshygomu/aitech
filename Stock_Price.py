    ...
    try:
        ...
        if result['totalSize'] > 0:
            ...
            def highlight_zero_quantity(row):
                return ['background-color: yellow' if row['数量'] != 0 else '' for _ in row]
...
if st.button("データ登録", disabled=not botao_confirmar_ativado):  # Texto do botão alterado
    try:

...
class VideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        logging.info("Frame recebido.")  # Verificação: Confirmação de que o frame foi recebido
        img = frame.to_ndarray(format="bgr24")

        if img is None:
            logging.warning("Nenhuma imagem capturada.")  # Verificação: Caso a imagem não seja capturada corretamente
            self.result_text = "No image captured"
            return av.VideoFrame.from_ndarray(img, format="bgr24")
        else:
            logging.info(f"Imagem capturada com dimensões: {img.shape}")  # Verificação: Dimensões da imagem capturada
        ...
        if points is not None and data:
            ...
            logging.info(self.result_text)  # Verificação: Exibe o QR code detectado
            ...
            logging.info("Nenhum QR Code detectado.")  # Verificação: Quando não há QR code detectado
